...
resource "azuread_application" "my_service_application" {
...
resource "azuread_service_principal" "book_service_principle" {
  application_id               = azuread_application.my_service_application.application_id
  app_role_assignment_required = false

}

resource "azuread_service_principal_password" "my_service_principle_password" {
  service_principal_id = azuread_service_principal.my_service_principle.id
...
resource "azurerm_role_assignment" "my-service-principle-role" {
  scope                = data.azurerm_subscription.primary.id
  role_definition_name = "Contributor"
  principal_id         = azuread_service_principal.my_service_principle.object_id
}
...
resource "azurerm_key_vault_access_policy" "my_owner_access_policy" {
  ...
  depends_on = [
    azurerm_role_assignment.my-service-principle-role,
    azuread_service_principal_password.my_service_principle_password,
    azuread_service_principal.my_service_principle
  ]
}
...
resource "azurerm_key_vault_secret" "sql-username" {
  ...
  key_vault_id = azurerm_key_vault.my_keyvault.id
    ...
    azuread_service_principal_password.my_service_principle_password,
    azuread_service_principal.my_service_principle,
    azurerm_key_vault_access_policy.my_owner_access_policy
  ]
}
